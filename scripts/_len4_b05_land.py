"""Land len4-b05: two-phase sample-parent commit then fail removal.
...
BATCH = "len4-b05-20260715"
SAMPLE_SEED = 20260720
PROMPT = ROOT / "data/syn_ant/project-antonyms-prompt-len4.txt"
HEADS = ROOT / "data/syn_ant/fixtures/len4_b05_unresolved.txt"
PARENT_FIXTURE = ROOT / "data/syn_ant/fixtures/len4_b05_parent_pairs.tsv"
NN_FIXTURE = ROOT / "data/syn_ant/fixtures/len4_b05_no_natural_final.tsv"
...
def _build_meta(
    *,
    ...
    return {
        ...
        "model_note": (
            "len4-b05 blind draft (maintainer-curated pairs + no_natural for opaque "
            "heads); A–D sample seed=20260720; soft-prefer 4-char tails"
        ),
        "model": "xai/grok-4",
        "model_provider": "xAI",
...
def phase_final(parent_commit: str) -> None:
    ...
    # drop heads already covered as project_ant head/tail in this campaign
    from ingest.project_antonyms_campaign import (
        accepted_coverage_heads,
        parse_campaign_manifest,
    )
    camp = {h.head for h in parse_campaign_manifest(
        LEN4_SPEC.manifest_tsv, meta_path=LEN4_SPEC.manifest_meta, spec=LEN4_SPEC
    )}
    covered_any = accepted_coverage_heads(DEFAULT_TSV) & camp
    rows = [(h, r, b) for h, r, b in rows if h not in covered_any]
