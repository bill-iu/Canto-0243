...
領域詞彙：見 [CONTEXT.md](../../CONTEXT.md) § 查詢分派、缺字型查詢執行。

缺字型家族的 ParsedQuery→MatchSpec 轉換與 `execute_mask_family_search` 內的 isinstance 分派梯，使新語法需同步改 `query_parse`、`query_dispatch`、`position_match` 三處。我們決定：**缺字型家族**正規化收進**查詢分派**（`QueryEngine`），**缺字型查詢執行**只接收正規化後的比對規格（`execute_match_spec`），不再依 ParsedQuery 型別分支；`HybridTailEqualsAlias` 等別名改寫亦在分派層完成。近反義關係查詢、近義複合詞、`WordLookup` 等路徑維持現有 executor。

**Considered Options**

- 正規化留在 `position_match` — 與「**查詢分派**」領域定義衝突，執行層持續綁語法型別。
- 執行層仍收 ParsedQuery、只換 registry 實作 — 正規化已上移但 seam 仍洩漏分類型別。
- 正規化與執行 registry 一次合併單 PR — review 與 rollback 面過大。
