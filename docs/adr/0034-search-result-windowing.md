...
1. **兩層分離**：**擷取頁**與 **呈現批次**（DOM 每次 +400 合併詞條）分開；統計「已擷取 N / 總數」只反映記憶體列數。
2. **首屏擷取 vs 上限**：**擷取頁上限** 800（load-more／續頁）；**0243搜尋模式**家族 **首屏擷取** 400（offset=0 首查）。近反義池沿用既有較細頁。雙端共用常數。
...
**Considered：** 首屏畫晒擷取頁——拒絕（卡頓）。首查亦滿上限——拒絕（連續查詢 wall time）。IO + scroll pump 連環——拒絕（自動拉完整庫）。每批 50——改 200 再改 400（減捲動次數）。上限 1200→800（平衡網絡／DOM）。PWA 改為只 submit 先搜——拒絕。無證據硬補 I2 索引——拒絕。