...
日誌請寫入 `logs/`（若腳本支援）或系統暫存區，並定期清理。**Desktop** 正式套件：Windows 只雙擊 `Canto-0243.exe`（無 `.bat`）；macOS 雙擊 **`Canto-0243.app`**（無 `.command`，ADR-0070）。clone 開發用 `./start.sh`（與 Desktop 交付分開；同樣開 `/app/`）。
