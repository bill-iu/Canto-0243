...
* **多种查询语法**：纯汉字 · 纯数字 · **粤拼查询** · **粤拼锚** · 混合码字（`23就`）· wildcard · **串列韵／声锚**（`04困=49倒=`、`23就=`）· **前缀通配等号**（`?香港=`）· 等号韵／声（`香港=`、`2=我3`）· 韵／声锚（`就=`、`?*就=`、`?港=?`）。
...
* **位置与通配符**：`香??`、`?你?`、`3_`、`23?`。
* **串列韵／声锚**：左至右每位数字一音节码；`{码}{字}=` 韵、`{码}={字}` 声（如 `23就=`、`04困=49倒=`、`?3人=?`）。
* **前缀通配等号**：`?{词≥2}=` 首音节通配＋整段韵模板（如 `?香港=`）。
...
**最后更新**：2026-06-17（v1.5.0 · 星号锚 · 教学整合 · 统一启动 · 粤拼用语）
