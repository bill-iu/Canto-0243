...
- **补充词库来源**：集成粤拼（Rime Cantonese 102K+ 字、330K+ 词）、HSK 3.0（11K 普通话参考词）、开放粤语词典，提高词汇覆盖
- **混合字符串统一处理**：新增 `LexiconCandidateNormalizer`，统一处理 CJK + 字母混合词条（如「ai2 Canto」）
- **词库完整性验证**：PWA 网络下载后自动以 SHA-256 + 字节大小验证，确保数据完整无损
- **TypeScript 修复**：升级兼容 TypeScript 6.0+，无弃用警告
