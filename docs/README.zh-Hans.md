...
官方离线资料包：**[Canto-0243 v1.6.0](https://github.com/bill-iu/Canto-0243/releases/tag/v1.6.0)**（`canto-0243-portable.zip`、macOS `tar.gz`、`lyrics.db`、`words-lexicon.json`）。问题与建议欢迎提交 [GitHub Issues](https://github.com/ICE-U-code/Canto-0243/issues)。
...
1. 从 [GitHub Releases](https://github.com/ICE-U-code/Canto-0243/releases) 下载 **`canto-0243-portable.zip`**（建议对照 [`Canto-0243 v1.6.0`](https://github.com/bill-iu/Canto-0243/releases/tag/v1.6.0)）。
2. 解压缩整个文件夹（例如 `canto-0243-portable`）。
3. 按平台启动：
   * **Windows**：解压后双击 **`START.bat`**（无需安装 Python）。
   * **macOS**：依芯片下载 `canto-0243-portable-macos-arm64.tar.gz` 或 `canto-0243-portable-macos-x86_64.tar.gz`，解压后双击 **`Canto-0243.app`**。
...
| `门0` | 二字：首字字面「门」＋尾格码 0（normalize 为 `+门0`） |

### 串列韵／声锚

左至右扫描：每位数字一音节码；`=` 永远在参考字右侧。`{码}{字}=` 比韵；`{码}={字}` 比声。可加 `?` 通配单槽。

| 输入示例 | 说明 |
|----------|------|
| `4困=` | 一字，与「困」押韵 |
| `04困=` | 二字，第 2 格与「困」押韵 |
| `23就=` | 二字，码 23＋尾格与「就」押韵 |
| `04困=49倒=` | 四字，第 2／4 格韵锚（穷困潦倒） |
| `0449穷困潦倒=` | 四字，码 0449＋整词同「穷困潦倒」韵（不同于串列：要求全词韵母） |
| `04=困49=倒` | 四字，第 2／4 格声锚 |
| `?3人=?` | 三字，中格码 3＋与「人」押韵 |
| `?4困=4潦=9倒=` | 四字，首槽通配＋韵锚 |
| `?困潦倒=` | 四字，首音节通配，其余与「困潦倒」押韵 |
| `穷?潦倒=` | 四字，第 1 格通配＋部分韵锚 |
| `穷困?倒=` | 四字，第 3 格通配＋部分韵锚 |
| `穷困潦=?` | 四字，尾格通配＋部分韵锚（normalize 为 `穷困潦?=`） |
| `?=困潦倒` | 四字，首音节通配＋声锚模板 |
| `=穷?潦倒` | 四字，第 2 格通配＋部分声锚 |
| `=穷困潦?` | 四字，尾格通配＋部分声锚 |
...
| 输入示例 | 说明 |
|----------|------|
| `?香港=` | 首音节通配，其余音节与「香港」押韵 |
| `?困潦倒=` | 首音节通配，其余与「困潦倒」押韵（须写尾 `=`） |

### 加号锚（`+`）— 指定某一格的「字面／同韵／同声」

用 `+`（slot 连接符）把「码」和「锚字」连接起来。**`=` 永远紧贴锚字后面**，表示该格和锚字**同韵母**；没有 `=` 则表示该格**字面固定**为锚字。`+=`（锚字在右侧）表示该格和锚字**同声母**。输入 `*`／`＊` 仍接受（silent 别名，normalize 为 `+`）。
...
> `门0` 仍可用，normalize 为 `+门0`。

（补充）同韵／同声锚：`香=?`、`+香=?`、`就=`（`?就=` normalize）、`=就`（`?=就` normalize）、`?+就=`、`?+=就`、`?+港=?`（`?港=?` normalize 等价）、`=香?`。
...
| 输入示例 | 说明 |
|----------|------|
| `?yut?` | 三字，**中格**韵母片段 `yut`（normalize 为 `?+yut?`） |
| `?syut?` | 三字，**中格**完整音节 `syut`（normalize 为 `?+syut?`） |
| `?hon` | 二字，**末格**完整音节 `hon`（normalize 为 `?+hon`） |
...
| `23+o` | 三字，码 `23`＋**末格**韵母 `o`（比二字 `23o` 多一槽） |
| `3h4` | 二字，码 `34`，**首格**声母 `h` |
