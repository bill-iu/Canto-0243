...
**Canto-0243**（**ONE·揾·韵**）是我在多种 AI Agent（Cursor、Codex、Grok Build、GitHub Copilot）协助下开发的离线粤语填词检索工作台：依据 **394052／02493 数字码**、**粤拼**、**韵母／声母规则**与 **近义／反义关系**，在数秒内列出符合条件的**词条**。顶栏 **0243搜索模式** 有三档：**0243模式**（松）、**02493模式**（紧）、**394052模式**（矩阵，三声 `4`／五声 `5` 分明）。例如输入 `23就` 可查找同调且与「就」押韵的尾字；输入 `香港=` 可查找与「香港」押韵的候选词；输入 `~开心` 或切换**近反义模式**可查找近义／反义词；输入 `~~`／`!!` 可查找填词常用的二字近义／反义复合词。套件解压即可使用，词库与近反义资料均存于本地，无需联网。
...
<!-- words-count:zh-Hans -->
目前总词条列数：**468,438**（`lyrics.db` · `words` 表）
<!-- /words-count:zh-Hans -->
...
* **0243搜索三档**：**0243模式** `mode=m1`（松档等价）· **02493模式** `mode=m2`（仅 `4↔5` 松档）· **394052模式** `mode=m3`（矩阵码逐位精确；词库存储用 394052，三声 `4`、五声 `5` 分明）。
...
**最后更新**：2026-07-04（重构 README，新增 Canto-0243 手机版与离线 portable 版本并大幅精简内容）
