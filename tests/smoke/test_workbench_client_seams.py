...
class WorkbenchClientSeamTests(unittest.TestCase):
    ...
    def test_phase1_closeout_guards(self) -> None:
        ...
        self.assertNotIn("candidate-scroll", cards)
        self.assertNotIn("scrollResetKey", cards)
        self.assertNotIn("scrollTop = 0", cards)
        self.assertNotIn("candidateScrollResetKey", page)
        self.assertNotIn("scrollResetKey={candidateScrollResetKey}", page)
        self.assertIn("workbench-return-to-input", page)
        self.assertIn("IntersectionObserver", page)
        self.assertIn("scrollIntoView", page)
        self.assertNotIn("lineInputFormRef.current?.focus", page)
        self.assertIn("span-hand-input__actions", canvas)
    ...
    def test_compact_entry_flow_is_shared_by_query_and_workbench(self) -> None:
        ...
        self.assertIn("flex: 0 0 calc((100% - 3.5rem) / 6)", workbench_css)
        self.assertIn("width: calc((100% - 3.5rem) / 6)", workbench_css)
        self.assertNotIn("grid-template-columns: repeat(6, minmax(0, 1fr))", workbench_css)
        self.assertNotIn(".candidate-scroll", workbench_css)
        self.assertNotIn("max-block-size: min(60dvh, 36rem)", workbench_css)
    ...
    def test_phase2_bridge_and_shortcuts(self) -> None:
        ...
        self.assertIn("useWorkbenchInput", page)
        self.assertIn("consumeIngest", (WORKBENCH / "useWorkbenchInput.ts").read_text(encoding="utf-8"))
        ...
        self.assertIn("tDetail('detail.putWorkbench'", detail)
        ...
        self.assertIn("useWorkbenchTransfer", app)
        self.assertIn("consumeNavigate", (WORKBENCH / "useWorkbenchTransfer.ts").read_text(encoding="utf-8"))
