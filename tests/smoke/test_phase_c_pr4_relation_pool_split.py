...
class PhaseCPr4RelationPoolSplit(unittest.TestCase):
    ...
    def test_runtime_callers_use_projection_not_builder(self):
        ...
        # only projection/builder package modules may import builder
        for path in (REPO / "client" / "src").rglob("*.ts"):
            ...
            if "buildRelationPool" in text or "relation-pool/builder" in text or "relation-pool-builder" in text:
                # self-check scripts live under client/scripts, not src
    ...
    def test_python_package_layout(self):
        ...
        self.assertNotIn("build_pool", init)  # builder not public barrel
