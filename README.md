...
| 詞庫 | **詞庫埠**：多字收錄靠**詞級標音**（上游詞表）；單字靠 rime **預設**讀音 |
| 排序 | 同 match tier：**純漢字** → **essay 詞頻** → **curated** → **pron_rank** → 字面（[`CONTEXT.md`](CONTEXT.md) § 搜尋結果排序） |
| Runtime | essay 啟動載入記憶體 dict；`word_relations` + static thesaurus 純 SQL |
...
```text
查詢字串 → parse_query · query_dispatch → handler（equals / hybrid / mask / syn …）
                ↓
...
| `app/services/query_parse.py` | 查詢字串 → ParsedQuery（純解析） |
| `app/services/query_dispatch.py` | ParsedQuery → handler 分派與執行 |
| `app/services/query_engine.py` | 向後相容 facade（parse + dispatch） |
...
| `app/services/manual_relation_service.py` | 關係補錄／撤回（manual `word_relations`） |
| `app/routers/relation.py` | 關係補錄 API |
| `app/domain/` | 收錄門檻、關係 canonical／建池領域規則 |
...
│   ├── domain/             # lexicon admission · relations canonical / pool
│   ├── lexicon/            # static 0243 · rime char · essay · curated
│   ├── thesaurus/          # runtime static 近反義索引
│   ├── services/           # 搜尋、排序、query parse/dispatch、syn/ant
│   ├── repositories/
...
目前 **220** 個 unittest（含 `test_jyutping_match`、`test_search_sort` 等）。
