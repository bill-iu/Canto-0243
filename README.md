...
* **多種查詢語法**：純漢字 · 純數字（分頁 + 總數 header）· **粵拼查詢**（`syut`／`nei hou`／`ming4 baak6`）· **粵拼錨**（`?yut?`、`23o`、`3hon4` 等拉丁錨，見速查）· 混合碼字（`23就`）· wildcard（`3_`、`23?`）· 等號韻／聲（`香港=`、`2=我3`）· 韻／聲錨（`?就=`、`?港=?`）。
...
* **粵拼錨**：缺字查詢內用拉丁取代漢字參考字（`?syut?` 中格音節、`23o` 碼後**末格**韻母、`3hon4` 首格音節等）；**唔係**整段粵拼查詢；**近反義模式**唔收。
* **近反義關係查詢**：`~開心`、`!你`、`33!開心`。
...
缺字查詢家族內以拉丁字母標讀音約束，**唔係**整段粵拼查詢（`syut` 查詞 ≠ `?syut?` 錨中格）。**近反義模式**唔收粵拼錨；無效韻母片段會拒絕並提示。
...
`3hon4` 音節喺**首格**；`23ngo`／`23o` 拉丁喺碼後、錨**末格**——兩者格式唔同，勿混。
