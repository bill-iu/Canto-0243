...
* **多種查詢語法**：純漢字 · 純數字（分頁 + 總數 header）· **粵拼查詢**（`syut`／`nei hou`／`ming4 baak6`）· **粵拼錨**（`?yut?`、`23o`、`3hon4` 等粵拼錨，見速查）· 混合碼字（`23就`）· wildcard（`3_`、`23?`）· 等號韻／聲（`香港=`、`2=我3`）· 韻／聲錨（`?就=`、`?港=?`）。
