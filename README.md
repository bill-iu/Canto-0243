...
**產品保證路徑**：離線單機 + **SQLite**（`lyrics.db` 或 `DATABASE_URL` 指向的 SQLite 檔）。新 schema 變更（欄位、索引、表）**僅**透過 SQLite bootstrap／`init_db.py` 維護；測試亦只覆蓋 SQLite。
...
| Ingest / dev | `requirements-dev.txt` | cilin、opencc、ML（`import_data`、`generate_relationships` 等） |
