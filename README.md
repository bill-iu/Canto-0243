...
官方離線資料包：**[Canto-0243 v1.6.3](https://github.com/bill-iu/Canto-0243/releases/tag/v1.6.3)**（`canto-0243-portable.zip`、macOS `tar.gz`、`lyrics.db`、`words-lexicon.json`）。問題與建議歡迎 [GitHub Issues](https://github.com/ICE-U-code/Canto-0243/issues)。
...
* **多種查詢語法**：純漢字 · 純數字 · **粵拼查詢** · **粵拼錨** · 混合碼字（`23就`）· wildcard · **串列韻／聲錨**（`04困=49倒=`、`23就=`）· **四字部分韻／聲錨**（`窮?潦倒=`、`=窮?潦倒`）· **前綴通配等號**（`?香港=`、`?=困潦倒`）· 等號韻／聲（`香港=`、`2=我3`）· 韻／聲錨（`就=`、`?+就=`、`?港=?`）· **加號錨**（`23+就=`）。
...
1. 從 [GitHub Releases](https://github.com/ICE-U-code/Canto-0243/releases) 下載 **`canto-0243-portable.zip`**（建議對照 [`Canto-0243 v1.6.3`](https://github.com/bill-iu/Canto-0243/releases/tag/v1.6.3)）。
2. 解壓縮整個資料夾（例如 `canto-0243-portable`）。
3. 依平台啟動：
   * **Windows**：解壓後雙擊 **`START.bat`**（無需安裝 Python）。
   * **macOS**：依晶片下載 `canto-0243-portable-macos-arm64.tar.gz` 或 `canto-0243-portable-macos-x86_64.tar.gz`，解壓後雙擊 **`Canto-0243.app`**。若系統顯示無法驗證／無法開啟，對 **`.app`** 或 **`Open Canto-0243.command`** **右鍵→打開**（各確認一次）；備用腳本會清隔離並啟動 `.app`。
...
* **串列韻／聲錨**：左至右每位數字一音節碼；`{碼}{字}=` 韻、`{碼}={字}` 聲（如 `23就=`、`04困=49倒=`、`?3人=?`）；**四字部分韻／聲錨**用 `?` 通配單格（如 `窮?潦倒=`、`=窮困?倒`）。
...
| `門0` | 二字：首字字面「門」＋尾格碼 0（normalize 為 `+門0`） |

### 串列韻／聲錨

左至右掃描：每位數字一音節碼；`=` 永遠在參考字右側。`{碼}{字}=` 比韻；`{碼}={字}` 比聲。可加 `?` 通配單槽。

| 輸入範例 | 說明 |
|----------|------|
| `4困=` | 一字，同「困」同韻 |
| `04困=` | 二字，第 2 格同「困」同韻 |
| `23就=` | 二字，碼 23＋尾格同「就」同韻 |
| `04困=49倒=` | 四字，第 2／4 格韻錨（窮困潦倒） |
| `0449窮困潦倒=` | 四字，碼 0449＋整詞同「窮困潦倒」韻（唔同串列：要求全詞韻母） |
| `04=困49=倒` | 四字，第 2／4 格聲錨 |
| `?3人=?` | 三字，中格碼 3＋同「人」同韻 |
| `?4困=4潦=9倒=` | 四字，首槽通配＋韻錨 |
| `?困潦倒=` | 四字，首音節通配，其餘同「困潦倒」同韻 |
| `窮?潦倒=` | 四字，第 1 格通配＋部分韻錨 |
| `窮困?倒=` | 四字，第 3 格通配＋部分韻錨 |
| `窮困潦=?` | 四字，尾格通配＋部分韻錨（normalize 為 `窮困潦?=`） |
| `?=困潦倒` | 四字，首音節通配＋聲錨模板 |
| `=窮?潦倒` | 四字，第 2 格通配＋部分聲錨 |
| `=窮困潦?` | 四字，尾格通配＋部分聲錨 |
...
| 輸入範例 | 說明 |
|----------|------|
| `?香港=` | 首音節通配，其餘音節同「香港」同韻 |
| `?困潦倒=` | 首音節通配，其餘同「困潦倒」同韻（須寫尾 `=`） |

### 加號錨（`+`）— 指定某一格的「字面／同韻／同聲」

用 `+`（slot 連接符）把「碼」和「錨字」連接起來。**`=` 永遠緊貼錨字後面**，表示該格和錨字**同韻母**；沒有 `=` 則表示該格**字面固定**為錨字。`+=`（錨字在右側）表示該格和錨字**同聲母**。輸入 `*`／`＊` 仍接受（silent 別名，normalize 為 `+`）。

**三種形狀，一眼看出格位：**

- **尾格**：`{code}+{漢字}{可選 '='}` / `{code}+= {漢字}`  
  例：`23+好`、`23+好=`、`23+=好`
- **中格**：`{左碼}+{漢字}{可選 '='}{右碼}`  
  例：`2+好3`（中格字面「好」）、`2+好=3`（中格同「好」同韻母）
- **頭格**：`+{漢字}{可選 '='}{右碼}`  
  例：`+門0`（首字字面「門」）、`+門=0`（首字同「門」同韻母）

> `門0` 仍可用，normalize 為 `+門0`。

（補充）同韻／同聲錨：`香=?`、`+香=?`、`就=`（`?就=` normalize）、`=就`（`?=就` normalize）、`?+就=`、`?+=就`、`?+港=?`（`?港=?` normalize 等價）、`=香?`。

### 粵拼錨

缺字查詢家族內以粵拼標讀音約束，**不是**整段粵拼查詢（`syut` 查詞 ≠ `?syut?` 錨中格）。**近反義模式**不支援粵拼錨；無效韻母片段會拒絕並提示。

**通配符位置**

| 輸入範例 | 說明 |
|----------|------|
| `?yut?` | 三字，**中格**韻母片段 `yut`（normalize 為 `?+yut?`） |
| `?syut?` | 三字，**中格**完整音節 `syut`（normalize 為 `?+syut?`） |
| `?hon` | 二字，**末格**完整音節 `hon`（normalize 為 `?+hon`） |

**碼 + 粵拼**

| 輸入範例 | 說明 |
|----------|------|
| `3hon4` | 二字，碼 `34`，**首格**完整音節 `hon` |
| `3?hon4` | 三字，`{首碼}+{音節}{末碼}`，中格音節 |
| `23+o` | 三字，碼 `23`＋**末格**韻母 `o`（比二字 `23o` 多一槽） |
| `3h4` | 二字，碼 `34`，**首格**聲母 `h` |
| `23ngo` | 二字，碼 `23`，**末格**完整音節 `ngo` |
| `23o` | 二字，碼 `23`，**末格**韻母 `o`（比 `23ngo` 闊） |
| `23ei0` | 三字，碼 `230`，**中格**韻母 `ei`（同 `23你=0` 類） |

`3hon4` 音節喺**首格**；`23ngo`／`23o` 粵拼喺碼後、錨**末格**——兩者格式唔同，勿混。
