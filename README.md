...
* **多種查詢語法**：純漢字 · 純數字 · **粵拼查詢** · **粵拼錨** · 混合碼字（`23就`）· wildcard · **串列韻／聲錨**（`04困=49倒=`、`23就=`）· **前綴通配等號**（`?香港=`）· 等號韻／聲（`香港=`、`2=我3`）· 韻／聲錨（`就=`、`?*就=`、`?港=?`）。
...
* **串列韻／聲錨**：左至右每位數字一音節碼；`{碼}{字}=` 韻、`{碼}={字}` 聲（如 `23就=`、`04困=49倒=`、`?3人=?`）。
* **前綴通配等號**：`?{詞≥2}=` 首音節通配＋整段韻模板（如 `?香港=`）。
...
用 `*` 把「碼」和「錨字」連接起來。**`=` 永遠緊貼錨字後面**，表示該格和錨字**同韻母**；沒有 `=` 則表示該格**字面固定**為錨字。`*=`（錨字在右側）表示該格和錨字**同聲母**（舊語法保留）。
...
> `門0` 仍可用，但屬 **deprecated** alias；新教學請用 `*門0`。
...
**最後更新**：2026-06-17（v1.5.0 · 星號錨 · 教學整合 · 統一啟動 · 粵拼用語）
