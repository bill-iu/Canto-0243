...
**漢字完整音節錨**：
**單個** `$` 後**僅**接單一漢字；該格約束為該字 rime **預設讀音**嘅完整音節字母（唔含聲調），語意等同對應**完整音節粵拼錨**（如 `3$漢4` ≡ `3hon4`）。**字位範圍**鏡像今日所有**完整音節粵拼錨**位（首格、末格、三字中格等）。全形 `＄` normalize 為 `$`。**連續 2–4 個 `$`** 屬 **雙聲疊韻字查詢**，入口 normalize **唔**拆開、**唔**改寫；單 `$`+漢字改寫僅喺唔屬連續 `$` 串時進行。多音字唔展開；預設讀音無效 → 拒絕並提示。
_Avoid_：`$` 後接拉丁粵拼、用非預設讀音展開 OR、把 `$` 當 slot 連接符、只支援首格而唔鏡像其他完整音節錨位、把 `$$$` 拆成 `$$`+`$` 再當音節錨

**雙聲疊韻字查詢**：
以連續 `$` 為核心標記（鏡像 **近義複合** `~~` 語法骨架）：**N 個 `$` = N 字詞**（**2≤N≤4**；`$` 單獨保留畀 **漢字完整音節錨**）；例 `$$`（二字）、`$$$`（三字）、`$$$$`（四字）。**N>4** → 拒絕並提示。搵該長度詞且**全部音節逐格聲母與韻母相同**（實作上即粵拼去聲調後各格音節字母一致；**唔**要求 N 個漢字係同一個字，如「識食」）。比對用該詞**收錄粵拼**逐音節；**任一讀音列**內全部音節字母相同即入選（**唔**要求所有異讀同時滿足）。結果**按字面合併**，**只展示符合條件嘅讀音列**（唔合格異讀唔顯示）。清單順序套用 **搜尋結果排序**（Essay 詞頻、Curated 常用詞、pron_rank 等），常用字面同常用讀音靠前。可選 `{碼}{N×$}`、`{N×$}{字}`、`{碼}{N×$}{字}` 縮窄：**碼位數須與 N 一致**（如 `333$$$`），唔一致 → 拒絕並提示；**尾字韻錨**縮窄尾字同參考字**同韻**（規則同 `~~` 尾錨，適用 N≥2）。候選來自 **離線啟動預載** 按詞長 **N=2、3、4 各一份** 快照；查詢時再套碼前綴與尾錨縮窄，**唔**查詢時裸掃全表。全形 `＄` 連續出現 normalize 為半形 `$`。**分派優先**：連續 `$`（2–4）整串原子匹配本查詢；`$` 個數唔喺 2–4 → 拒絕；單 `$`+漢字留畀 **漢字完整音節錨**（如 `$$漢` 係本查詢尾字韻錨，唔係音節錨）。碼前綴受 **0243搜尋模式** m1/m2 等價約束。**近反義關係查詢**唔收。
_Avoid_：同音節疊字查詢（舊正名）、逐字 rime 預設代替詞級標音、要求所有讀音列同時滿足、把連續 `$` 當漢字完整音節錨、與 `~~` 近義語意混淆、把查詢誤解為古典「只雙聲」或「只疊韻」而唔係全詞各格音節相同、要求字面重複（如只收「慢慢」唔收「識食」）

**同音異讀查詢**：
以 `{左碼}/{右碼}` 為標記；搵**同一字面**且詞庫內存在**至少兩個唔同讀音列**，分別符合左、右碼位模板（m1/m2 **0243搜尋模式**等價）。左右**等長**；已寫數字約束該碼位，未寫位以 **`?`** 通配（如 `?3/?4` 只約束第 2 字碼；`33/34` 二字四位全寫死）。詞長 = 模板長度；不等長 → 拒絕並提示。入選須**至少一列符合左、至少一列符合右**，且參與嘅左右兩列**粵拼唔同**。結果**按字面合併**，每條讀音標註符合**左**／**右**／**左·右**。清單順序：不同字面按**搜尋結果排序**的頻率部分（essay 詞頻、curated 等），同字面內讀音按粵拼 lexical 排序（恢復舊行為）。候選來自**啟動預載記憶體索引**（`char` → 多讀音列；只含 `COUNT(DISTINCT jyutping) > 1` 嘅字面；**唔**增大 `lyrics.db`）；查詢時做碼位模板過濾。**僅 0243搜尋模式**；整段須符合 `^[\d?]+/[\d?]+$`（全形 `／` normalize 為 `/`）；含漢字或其他符號 → 拒絕並提示。**近反義關係查詢**唔收。
...
**本機啟動**：
維護者 `./start.sh` 與創作者 **Portable 套件**內 `START.bat`／`START.sh`／`.app` 共用同一啟動契約：（1）**終端／視窗須在執行后立刻**顯示「正在啟動」類回饋（Windows 英文、其他平台可中英並列），避免雙擊後數秒無任何可見反應——**GUI 啟動器**成功時除外；（2）背景程序就緒且**查韻介面**可載入後儘快開啟瀏覽器，**不**等待**就緒閘**解鎖；（3）頁面由既有就緒閘接手進度與搜尋解鎖。不得出現瀏覽器原生「無法連線」畫面——須確保 HTTP 已能回應查韻介面後才開啟 URL。維護者應在開發版啟動路徑驗證與 Portable 相同的體感延遲（含查韻介面就緒時間），不可只在 zip 內手測。
_Avoid_：等 gate_ready 才開瀏覽器、先開瀏覽器再等後端（未就緒時 fetch 失敗）、開發版與 Portable 啟動行為長期分叉、雙擊後長時間無終端回饋

**GUI 啟動器**：
Windows **Portable 套件**內 **`Canto-0243.exe`** 的啟動路徑。成功時無 cmd 視窗、查韻介面就緒後開瀏覽器；**失敗**時以系統對話框提示創作者（成功不彈窗）。若本機後端已在運行，可**後端沿用**（只開瀏覽器）。`START.bat` 仍作進階備用與終端診斷。
_Avoid_：失敗仍完全靜默、要求創作者讀 log 才懂失敗原因
