...
**煙霧測試套件**：
每次 PR 必過嘅精簡驗收；覆蓋核心查詢旅程、近反義池不變量、詞庫收錄與啟動就緒，主資料來源為 **詞條庫 fixture**。**黃金查詢集** 嘅 CI 子集屬其中一部份（約二十條查詢旅程）。**唔**等同完整 **黃金查詢集**；架構靜態契約由另類檢查承擔，**唔**計入煙霧條數目標。
_Avoid_：把大量 parser golden 當煙霧、CI 每次跑完整 **詞條庫建置命令**、把煙霧當 **全量發佈** 唯一驗收

**黃金查詢集**：
**詞級標音副件** 或查詢行為重大更新後，鎖定**零回歸**的固定查詢清單（含 **等號查詢**、成語、常見 2–3 字等）。**CI smoke 子集**（約二十條）每次 PR 必過，由 **煙霧測試套件** 查詢旅程承載；清單來源合併 maintainer bench 關鍵 case、各 **查詢分派** kind 代表查詢，去重後維護。**完整集**於 **Beta 預發佈**／**全量發佈**前必過（有本機 **詞條庫** 時跑 bench 或同等腳本）。
...
_Avoid_：一失敗就跳至新依賴、轉換到另一種資料庫引擎作主要路徑、為分析型 workload 另開第二資料庫引擎
...
**靜態客戶端束**：
**PWA 交付頻道**的核心交付物：純靜態檔案（HTML、JS、CSS、資源檔）的客戶端套件，無需後端伺服器即可於瀏覽器中執行；內含 **瀏覽器查詢引擎**、**sql.js 執行環境**與 **PWA 運行時**。以 `client/` 目錄為根，由 Vite 打包產出至 `dist/`，部署至 GitHub Pages。
_Avoid_：SPA（強調單頁不精確）、web app（泛稱）、前端套件（易與既有 Portable 前端混淆）

**瀏覽器查詢引擎**：
於瀏覽器中執行的查詢邏輯實現，將原 Python 查詢引擎的核心功能（0243 解析、模式比對、近反義關係、排名信號等）遷移至 JavaScript；透過 **sql.js** 直接操作 **lyrics.db** 靜態資產，提供與原生 CLI 相同的查詢體驗。
_Avoid_：JS 查詢引擎、前端查詢（與 **查詢分派** 等混淆）、把瀏覽器查詢引擎當成獨立產品（須結合 **靜態客戶端束** 執行）

**PWA 交付頻道**：
以 **Progressive Web App** 形式交付 **靜態客戶端束** 的新分發管道；支援「加到主畫面」（iOS）與「安裝為應用程式」（Android/桌面）；首次載入後透過 **Service Worker** 緩存所有資源（含 **lyrics.db**），達成完全離線可用。**GitHub Pages** 為主要部署目標。
_Avoid_：PWA 版（作產品名稱的唯一形態）、網頁版（忽略離線能力）、把 **PWA 交付頻道** 當成取代 **Portable 套件** 的唯一路徑

**PWA 運行時**：
**靜態客戶端束** 在瀏覽器中的執行環境，包含 **Service Worker** 管理的離線緩存、Web App Manifest 定義的安裝體驗、與 **瀏覽器查詢引擎** 的執行上下文。
_Avoid_：PWA environment、瀏覽器執行期

**服務工作者（Service Worker）**：
在瀏覽器背景執行的腳本，負責 **PWA 交付頻道** 的離線緩存策略：攔截網路請求、緩存 **lyrics.db** 與其他靜態資源、提供離線回應。採用 **Cache First** 策略確保已緩存資源可離線存取。
_Avoid_：SW（縮寫作正名）、service worker（全小寫作名詞）、工作者腳本
