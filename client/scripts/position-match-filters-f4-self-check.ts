/** ponytail: MF-5 F4 — equals_span filter parity vs executors */
...
if (!fs.existsSync(fixture)) {
  throw new Error(`position-match-filters-f4-self-check: missing ${fixture}`);
}
...
const cases = ['香港=', '?困潦倒=', '2=我3', '就='] as const;

for (const q of cases) {
  const parsed = normalizeAndParse(q);
  const spec = buildMatchSpecForParsed(parsed);
  if (!spec) {
    throw new Error(`position-match filters F4: no spec for ${q}`);
  }
...
db.close();
console.log('position-match filters F4 self-check ok');
