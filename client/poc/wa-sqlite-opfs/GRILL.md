...
- [x] OPFS 與 SW `CacheFirst` 雙路復原優先順序（DB-4）：**OPFS → SW cache → 網路**；見 `lexicon-restore.ts` 與 `offline-readiness.md`
- [x] `VITE_DB_BACKEND=opfs` 可選（DB-3）；預設 `sqljs`；**翻預設等 DB-5 gate**（research.md 實機表 + 兩版 release）
