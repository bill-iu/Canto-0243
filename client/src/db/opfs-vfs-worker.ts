...
export type OpfsVfsWorkerRequest =
  | { id: number; type: 'init'; fileName: string; dbUrl: string }
...
async function streamFetchIntoOpfs(fileName: string, dbUrl: string): Promise<number> {
...
async function ensureDbFile(fileName: string, dbUrl: string): Promise<{ byteSize: number; fetched: boolean }> {
  ...
  const byteSize = await streamFetchIntoOpfs(fileName, dbUrl);
  if (byteSize <= 0) {
...
self.onmessage = async (event: MessageEvent<OpfsVfsWorkerRequest>) => {
  ...
  try {
    if (msg.type === 'init') {
      const ensured = await ensureDbFile(msg.fileName, msg.dbUrl);
