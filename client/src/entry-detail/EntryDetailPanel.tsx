...
export function EntryDetailPanel({
  ...
  const [readingIdx, setReadingIdx] = useState(() =>
    pickPreferredReadingIndex(model.readings, preferredJyutping ?? undefined),
  );
  ...
  const handleCopy = useCallback(async () => {
    try {
      await navigator.clipboard.writeText(model.literal);
      setCopied(true);
    ...
    <aside className="entry-detail-panel" aria-label={tDetail('detail.title', lang)}>
      ...
      <div className="entry-detail-panel__body">
        <div className="entry-detail-panel__hero">
          <div className="entry-detail-panel__literal">{model.literal}</div>
        ...
        {model.syns.length ? (
          <section className="entry-detail-section">
            ...
            <div className="entry-detail-chip-row">
              {model.syns.map((word) => (
                <button key={word} type="button" className="entry-detail-chip" onClick={() => onRelationPick(word)}>
                  {word}
                </button>
              ))}
            </div>
