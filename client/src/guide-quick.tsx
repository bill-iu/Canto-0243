...
const GUIDE_QUICK_ROWS: GuideQuickRow[] = [
  ...
  },
  {
    ...
    blurbZh: '同韻用字後 =；同聲用字前 ^（舊字前 = 仍相容）；用 + 加長（? 後通常可省）。',
    blurbEn: 'Rhyme: trailing =; initial: leading ^ (legacy leading = still works); + to lengthen (often skip after ?).',
    examples: [
      { query: '香=', mode: '0243', labelZh: '同韻', labelEn: 'Same rhyme' },
      { query: '^香', mode: '0243', labelZh: '同聲', labelEn: 'Same initial' },
      { query: '=香', mode: '0243', labelZh: '兼容原本舊語法（=聲）', labelEn: 'Legacy leading = initial' },
  ...
  },
  {
    ...
    blurbZh: '唔知嘅位留空；? 後面通常唔使再打 +。可以混聲調數字同定實嘅漢字。',
    blurbEn: 'Leave unknowns open; after ? you usually omit +. Mix tone digits with fixed characters.',
    examples: [
      ...
      { query: '?你?', mode: '0243', labelZh: '中間定實「你」', labelEn: 'Lock middle 你' },
...
const GUIDE_QUICK_WIDE_COLUMNS: GuideQuickWideColumn[] = [
  {
    rows: [
      ...
      },
      {
        ...
        titleZh: '同韻／同聲／加長',
        titleEn: 'Rhyme / initial / lengthen',
        blurbZh: '同韻用字後 =；同聲用字前 ^（舊字前 = 仍相容）；用 + 加長。',
        blurbEn: 'Rhyme: trailing =; initial: leading ^ (legacy = still works); + to lengthen.',
        examples: [
          { query: '香=', mode: '0243', labelZh: '同韻', labelEn: 'Same rhyme' },
          { query: '^香', mode: '0243', labelZh: '同聲', labelEn: 'Same initial' },
          { query: '=香', mode: '0243', labelZh: '兼容原本舊語法（=聲）', labelEn: 'Legacy leading = initial' },
      ...
      },
      {
        ...
        blurbZh: '連續打數字＝每個字一個聲調；字後 = 同韻，字前 ^ 同聲。',
        blurbEn: 'Digits in a row = one tone each; char= rhyme; ^char initial.',
        examples: [
